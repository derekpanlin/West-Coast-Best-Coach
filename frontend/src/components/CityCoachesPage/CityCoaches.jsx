...
import { clearCoaches, fetchCoachesByCity } from "../../redux/coach";
...
const CityCoaches = () => {
  ...
  const capitalizedCity = formattedCity
    .split(" ")
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");

  const coaches = useSelector((state) =>
    Object.values(state.coach.coaches).filter(
      (coach) =>
    ...
    <div>
      ...
      <div className="coach-list">
        {sortedCoaches.length > 0 ? (
            ...
            <div key={coach.id} className="coach-card">
              <div className="coach-info">
                ...
                <div>
                  ...
                  <p>
                    <strong>Rate</strong>: $
                    {Number(coach.rate).toLocaleString()} / hour
                  </p>
                  <p>{coach.experience_years}+ years of experience</p>
                </div>
              </div>
              <div className="coach-actions">
                <Link to={`/coaches/${coach.id}`} className="view-profile-btn">
                  View Profile
                </Link>
                <button
          ...
          <p id="no-coaches-available">
            No coaches available in {capitalizedCity}.
          </p>
        )}
      </div>
    </div>
